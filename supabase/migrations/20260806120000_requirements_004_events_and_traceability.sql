-- REQUIREMENTS 004: traceability + event vocabulary for Requirements.
--
-- Frozen sources: AUTH 001-009, STORAGE 001-004, REALTIME 001-002,
-- REQUIREMENTS 002/003.
--
-- Delivers exactly:
--   1) Four canonical events emitted by the requirement.* workflow RPCs
--      per REQUIREMENTS 001 §17:
--        requirement.created / .updated / .archived / .assessed
--   2) Three UI-facing read RPCs per REQUIREMENTS 001 §21:
--        list_project_requirements
--        list_applicable_requirements
--        get_version_assessments
--
-- The requirement.* workflow RPCs (REQUIREMENTS 003) are re-created in place
-- to add event-emission blocks. All signatures, capability gates, state guards,
-- and OUT columns are preserved byte-for-byte from REQUIREMENTS 003. The only
-- behavioral change is: on a state-changing success, an activity_events row
-- is inserted before the RPC returns.
--
-- Traceability: no new schema. The nullable requirement_id columns already
-- exist on changes and decisions from REQUIREMENTS 002. Reads use standard
-- RLS. The list RPCs return only rows the caller is authorized to see.
--
-- Event conventions (mirrors STORAGE 003/004 + publish_version):
--   * subject_snapshot carries the semantic payload (code, changed_fields, …)
--   * payload column stays '{}'::jsonb
--   * actor_kind='user', actor_profile_id=auth.uid()

------------------------------------------------------------------------------
-- 1. create_requirement — emits requirement.created
------------------------------------------------------------------------------
...
begin
  ...
  -- REQUIREMENTS 004: canonical requirement.created event
...
------------------------------------------------------------------------------
-- 2. edit_requirement — emits requirement.updated iff any field changed
------------------------------------------------------------------------------
-- Diff is computed against the pre-UPDATE row. Zero-change edit → no event
-- and out_updated=false. This is stricter than REQUIREMENTS 003 which returned
-- true unconditionally; the change is (a) intentional to keep the event feed
-- meaningful, and (b) additive to the RPC contract (out_updated stays boolean).
...
------------------------------------------------------------------------------
-- 3. archive_requirement — emits requirement.archived on new archive
------------------------------------------------------------------------------
-- Idempotent behavior preserved: already-archived returns false and emits
-- NO event (no state actually changed).
...
begin
  ...
  insert into public.activity_events
    (workspace_id, project_id, occurred_at, event_type,
     ...
     subject_snapshot, payload)
  values (v_ws, v_pj, now(), 'requirement.archived',
          v_caller, 'user', 'requirement', p_requirement_id, v_code,
          jsonb_build_object('code', v_code, 'previous_status', v_status),
          '{}'::jsonb);
...
------------------------------------------------------------------------------
-- 4. supersede_requirement — emits requirement.updated (kind='supersede')
------------------------------------------------------------------------------

create or replace function public.supersede_requirement(
  p_old_requirement_id uuid, p_new_requirement_id uuid
) returns boolean language plpgsql security definer set search_path = ''
as $$
declare
  v_caller uuid;
...
begin
  ...
  select workspace_id, project_id, status, code
    into v_old_ws, v_old_pj, v_old_st, v_old_code
  ...
  select workspace_id, project_id, status, code
    into v_new_ws, v_new_pj, v_new_st, v_new_code
  ...
  insert into public.activity_events
    (workspace_id, project_id, occurred_at, event_type,
     actor_profile_id, actor_kind, subject_kind, subject_id, subject_label,
     subject_snapshot, payload)
  values (v_old_ws, v_old_pj, now(), 'requirement.updated',
          v_caller, 'user', 'requirement', p_old_requirement_id, v_old_code,
          jsonb_build_object(
            'code', v_old_code,
            'kind', 'supersede',
            'previous_status', v_old_st,
            'new_status', 'superseded',
            'superseded_by_requirement_id', p_new_requirement_id,
            'superseded_by_code', v_new_code
          ),
          '{}'::jsonb);
...
------------------------------------------------------------------------------
-- 5. set_requirement_applicability — emits requirement.updated (kind='applicability')
------------------------------------------------------------------------------
...
as $$
declare
  v_caller uuid; v_ws uuid; v_pj uuid; v_status text; v_parent uuid; v_code text;
  ...
  if v_added > 0 or v_removed > 0 then
    ...
    values (v_ws, v_pj, now(), 'requirement.updated',
            ...
            jsonb_build_object(
              ...
              'removed', v_removed,
              'kept', v_kept,
              'now_project_wide', (v_added + v_kept = 0)
            ),
            '{}'::jsonb);
  end if;
...
------------------------------------------------------------------------------
-- 6. assess_version_requirement — emits requirement.assessed on create OR update
------------------------------------------------------------------------------
-- Snapshot carries the previous status when we're overwriting a prior
-- assessment, so downstream feeds can render "changed from partial → satisfied".
...
begin
  ...
  select workspace_id, project_id, status, code into v_r_ws, v_r_pj, v_r_st, v_r_code
  ...
  -- Capture the previous status (if any) for the event payload
...
------------------------------------------------------------------------------
-- 7. list_project_requirements — read RPC
------------------------------------------------------------------------------
-- Returns every requirement in the project the caller can see, optionally
-- including archived and superseded. Gated by requirement.view. SECURITY
-- DEFINER + explicit capability check (RLS is bypassed under DEFINER but the
-- capability check enforces the same boundary).

create or replace function public.list_project_requirements(
  p_project_id           uuid,
  p_workspace_id         uuid,
  p_include_archived     boolean default false,
  p_include_superseded   boolean default true
)
returns table (
  ...
  out_superseded_by_requirement_id uuid,
...
begin
  ...
  return query
    select r.id, r.code, r.title, r.description, r.category, r.source, r.source_ref,
           r.status, r.parent_requirement_id, r.superseded_by_requirement_id,
           r.archived_at, r.created_by_profile_id, r.created_at, r.updated_at
     ...
     order by
       -- Roots first, sub-requirements grouped under their parent by shared prefix,
       -- then by code lexicographic within group.
       coalesce((select code from public.requirements p where p.id = r.parent_requirement_id), r.code),
       r.parent_requirement_id nulls first,
       r.code;
end $$;
...
------------------------------------------------------------------------------
-- 8. list_applicable_requirements — read RPC
------------------------------------------------------------------------------
-- Returns requirements that apply to a specific design asset. Sub-requirements
-- inherit their parent's applicability (matching the vra_enforce_applicability
-- trigger). If p_asset_version_id is provided, each row also carries the
-- caller's assessment status (or NULL if not yet assessed).
...
begin
  ...
  if p_asset_version_id is not null then
    -- Ensure the version belongs to the asset (defense-in-depth)
    perform 1 from public.asset_versions
     where id = p_asset_version_id and design_asset_id = p_design_asset_id;
    if not found then
  ...
  return query
    with root_scoped as (
      -- The effective (root) requirement for each row + whether it is scoped
      select r.id, r.code, r.title, r.status, r.parent_requirement_id,
             coalesce(r.parent_requirement_id, r.id) as effective_root_id
        from public.requirements r
       where r.project_id = v_pj
         and r.status in ('draft','active')  -- superseded/archived not applicable
    ),
    scoped_root_ids as (
      select distinct requirement_id as root_id
        from public.requirement_design_assets
       where design_asset_id = p_design_asset_id
    ),
    all_project_roots as (
      select id from public.requirements
       where project_id = v_pj
         and parent_requirement_id is null
         and status in ('draft','active')
    ),
    root_wide as (
      -- Roots that have no applicability entries at all => project-wide
    ...
    applicable_root_ids as (
      ...
      union
      select root_id as id from scoped_root_ids
    )
    select rs.id, rs.code, rs.title, rs.status, rs.parent_requirement_id,
      ...
      left join public.version_requirement_assessments a
             on a.asset_version_id = p_asset_version_id
            and a.requirement_id   = rs.id
...
------------------------------------------------------------------------------
-- 9. get_version_assessments — read RPC
------------------------------------------------------------------------------
-- Returns every assessment recorded against a specific asset_version with
-- basic requirement metadata for rendering.

create or replace function public.get_version_assessments(
...
begin
  ...
  return query
    select a.id, a.requirement_id, r.code, r.title, a.status, a.note,
           a.assessed_by_profile_id, a.assessed_at
