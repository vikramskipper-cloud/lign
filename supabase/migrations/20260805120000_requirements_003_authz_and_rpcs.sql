-- REQUIREMENTS 003: authorization + workflow RPCs for Requirements.
--
-- Frozen sources: AUTH 001-009, STORAGE 001-004, REALTIME 001-002,
-- REQUIREMENTS 002.
--
-- Delivers exactly:
--   1) lign_has_capability extended with 5 new keys per REQUIREMENTS 001 §14:
--        requirement.view / .create / .edit / .archive / .assess
--   2) The six workflow RPCs specified in REQUIREMENTS 001 §21:
--        create_requirement, edit_requirement, archive_requirement,
--        supersede_requirement, set_requirement_applicability,
--        assess_version_requirement
--
-- Non-goals in this slice: events, realtime, storage, frontend. The RPCs
-- perform their state transitions and rely on the frozen schema triggers /
-- CHECKs / composite FKs for structural integrity. No activity_events rows
-- are emitted here — REQUIREMENTS 004 owns the event vocabulary.
--
-- All new RPCs: SECURITY DEFINER, SET search_path = '', REVOKE from public
-- / anon, GRANT to authenticated + service_role. OUT columns are out_-prefixed
-- to avoid PL/pgSQL variable collision (STORAGE 003/004 convention).

------------------------------------------------------------------------------
-- 1. Extend lign_has_capability
------------------------------------------------------------------------------
-- Additive: (a) add requirement.view to the admin-override view-class set,
-- (b) extend the per-role allowlists per REQUIREMENTS 001 §14. All previously
-- accepted keys and role mappings preserved byte-for-byte.
...
begin
  ...
  if p_capability_key = any (array[
    ...
    -- REQUIREMENTS 003: requirement.view is a view-class capability that
    -- workspace admins get via override (mirrors project.view / asset.view).
    'requirement.view'
  ]) then
  ...
  return case v_role
    ...
    when 'contributor' then p_capability_key = any (array[
      ...
      'activity.view',
      -- REQUIREMENTS 003: contributors can view / create / edit / assess
      -- but NOT archive (preserves the frozen creative-vs-lead split).
    ...
    when 'reviewer' then p_capability_key = any (array[
      ...
      'file.download',
      'activity.view',
      -- REQUIREMENTS 003: reviewers may view + assess.
    ...
    when 'approver' then p_capability_key = any (array[
      ...
      'file.download',
      'activity.view',
      -- REQUIREMENTS 003: approvers may view + assess.
      'requirement.view','requirement.assess'
    ])
    when 'observer' then p_capability_key = any (array[
      ...
      'change.view','decision.view','approval.view','release.view',
      'file.download','activity.view',
      -- REQUIREMENTS 003: observers may view only.
      'requirement.view'
    ])
    else false
  end;
...
------------------------------------------------------------------------------
-- Server-generates a stable code (per REQUIREMENTS 001 §3):
--   * Root: R-001, R-002, … (3-digit zero-padded)
--   * Sub:  <parent_code>.1, <parent_code>.2, …
-- Serialization via per-project advisory xact lock prevents concurrent
-- INSERTs from computing the same code. Codes are never reassigned.
...
begin
  ...
  if v_caller is null then
    raise exception 'create_requirement: authentication required' using errcode='42501';
  end if;
  if p_project_id is null or p_workspace_id is null then
    raise exception 'create_requirement: project_id and workspace_id required' using errcode='22004';
  end if;
  if p_title is null or length(trim(p_title))=0 then
    raise exception 'create_requirement: title required' using errcode='22004';
  end if;
  if p_status not in ('draft','active') then
    raise exception 'create_requirement: initial status must be draft or active (got %)', p_status using errcode='22023';
  end if;
  ...
  -- Serialize code assignment within this project. Xact-scoped; released on commit/rollback.
  ...
  if p_parent_requirement_id is null then
    ...
    -- Sub-numbering: verify parent exists in this project, then extend its code.
    select code into v_parent_code
      from public.requirements
     where id = p_parent_requirement_id and project_id = p_project_id;
    if v_parent_code is null then
      raise exception 'create_requirement: parent % not found in project %',
        p_parent_requirement_id, p_project_id using errcode='23503';
    end if;
    select coalesce(count(*), 0) + 1 into v_n
      from public.requirements
...
comment on function public.create_requirement(uuid, uuid, text, text, text, text, text, uuid, text) is
  'REQUIREMENTS 003: creates a new requirement with server-generated code (root R-NNN or sub <parent>.<n>). Serialized per project via advisory xact lock. Requires requirement.create. No event emitted (REQUIREMENTS 004).';
...
-- 3. edit_requirement
------------------------------------------------------------------------------
-- Mutates non-structural fields only. Null parameter = leave field unchanged
-- (except for description/category/source/source_ref which accept an explicit
-- null via the sentinel value '__null__' — kept simple: pass current value or
-- new value; to clear, set to empty string which is stored as-is).
--
-- Status transitions allowed here: draft ↔ active. Archive and supersede have
-- their own RPCs so the state machine is explicit and audit-friendly.
...
begin
  v_caller := auth.uid();
  if v_caller is null then
    raise exception 'edit_requirement: authentication required' using errcode='42501';
  end if;
  if p_requirement_id is null then
    raise exception 'edit_requirement: requirement_id required' using errcode='22004';
  end if;

  select workspace_id, project_id, status
    into v_ws, v_pj, v_cur_status
    from public.requirements where id = p_requirement_id for update;
  if v_ws is null then
    raise exception 'edit_requirement: requirement % not found', p_requirement_id using errcode='23503';
  end if;
...
comment on function public.edit_requirement(uuid, text, text, text, text, text, text) is
  'REQUIREMENTS 003: edits non-structural fields on a requirement (title, description, category, source, source_ref) and toggles draft ↔ active. Blocks edits to superseded/archived rows. Requires requirement.edit.';
...
------------------------------------------------------------------------------
-- 4. archive_requirement
------------------------------------------------------------------------------
-- Terminal transition. draft/active → archived. Idempotent for already-archived.
-- Blocked for superseded (that path is terminal via supersede).

create or replace function public.archive_requirement(
  p_requirement_id uuid
)
returns boolean
language plpgsql
...
begin
  v_caller := auth.uid();
  if v_caller is null then
    raise exception 'archive_requirement: authentication required' using errcode='42501';
  end if;
  ...
  select workspace_id, project_id, status
    ...
    from public.requirements where id = p_requirement_id for update;
  if v_ws is null then
    raise exception 'archive_requirement: requirement % not found', p_requirement_id using errcode='23503';
  end if;
  ...
  if v_status = 'archived' then
    return false; -- already archived; idempotent success
  end if;
...
comment on function public.archive_requirement(uuid) is
  'REQUIREMENTS 003: transitions a requirement from draft/active to archived. Idempotent for already-archived. Superseded rows cannot be archived. Requires requirement.archive.';
...
------------------------------------------------------------------------------
-- Marks p_old_requirement_id as superseded by p_new_requirement_id. Both must
-- be in the same project. The replacement must be in a live state
-- (draft or active) and not itself superseded or archived.
-- Requires requirement.edit (semantic edit rather than a new capability).

create or replace function public.supersede_requirement(
  p_old_requirement_id uuid,
  p_new_requirement_id uuid
)
returns boolean
language plpgsql
security definer
...
begin
  v_caller := auth.uid();
  if v_caller is null then
    raise exception 'supersede_requirement: authentication required' using errcode='42501';
  end if;
  if p_old_requirement_id is null or p_new_requirement_id is null then
    raise exception 'supersede_requirement: both requirement ids required' using errcode='22004';
  end if;
  if p_old_requirement_id = p_new_requirement_id then
    raise exception 'supersede_requirement: cannot supersede a requirement with itself' using errcode='23514';
  end if;
  ...
  if v_old_ws is null then
    raise exception 'supersede_requirement: requirement % not found', p_old_requirement_id using errcode='23503';
  end if;
  ...
  if v_new_ws is null then
    raise exception 'supersede_requirement: replacement % not found', p_new_requirement_id using errcode='23503';
  end if;
...
comment on function public.supersede_requirement(uuid, uuid) is
  'REQUIREMENTS 003: marks an old requirement as superseded by a replacement in the same project. Both directions gated by same-project + status coherence. Requires requirement.edit.';
...
------------------------------------------------------------------------------
-- 6. set_requirement_applicability
------------------------------------------------------------------------------
-- Replaces the applicability set for a requirement. Empty/NULL array =
-- project-wide (delete all rows). Otherwise, each design_asset_id must be
-- in the requirement's project; existing rows not in the set are deleted;
-- missing entries are added. Composite FKs enforce workspace/project
-- coherence per REQUIREMENTS 002.

create or replace function public.set_requirement_applicability(
  p_requirement_id  uuid,
  p_design_asset_ids uuid[]
)
returns table (
  out_requirement_id uuid,
  out_added   integer,
...
begin
  v_caller := auth.uid();
  if v_caller is null then
    raise exception 'set_requirement_applicability: authentication required' using errcode='42501';
  end if;
  if p_requirement_id is null then
    raise exception 'set_requirement_applicability: requirement_id required' using errcode='22004';
  end if;
  ...
  select workspace_id, project_id, status, parent_requirement_id
    ...
    from public.requirements where id = p_requirement_id for update;
  if v_ws is null then
    raise exception 'set_requirement_applicability: requirement % not found', p_requirement_id using errcode='23503';
  end if;
  ...
  select count(*)::int into v_kept
   ...
   where requirement_id = p_requirement_id
     and design_asset_id = any (v_ids);

  -- Insert new (missing) links. Composite FK will reject any asset_id not in
  -- the requirement's (project_id, workspace_id).
  with added as (
    insert into public.requirement_design_assets
      (requirement_id, design_asset_id, workspace_id, project_id)
    select p_requirement_id, x, v_ws, v_pj
...
comment on function public.set_requirement_applicability(uuid, uuid[]) is
  'REQUIREMENTS 003: replaces the applicability set for a root requirement. Empty array = project-wide. Sub-requirements inherit from parent and cannot set their own. Existing assessments are preserved as historical records. Requires requirement.edit.';
...
------------------------------------------------------------------------------
-- 7. assess_version_requirement
------------------------------------------------------------------------------
-- Upsert per (asset_version_id, requirement_id). Applicability enforced by
-- the frozen trigger vra_enforce_applicability (REQUIREMENTS 002).
-- assessed_by is always the caller.
...
returns table (
  out_assessment_id uuid,
  out_action        text  -- 'created' | 'updated'
)
language plpgsql
security definer
set search_path = ''
...
begin
  v_caller := auth.uid();
  if v_caller is null then
    raise exception 'assess_version_requirement: authentication required' using errcode='42501';
  end if;
  ...
  if v_v_ws is null then
    raise exception 'assess_version_requirement: asset_version % not found', p_asset_version_id using errcode='23503';
  end if;

  select workspace_id, project_id, status into v_r_ws, v_r_pj, v_r_st
    from public.requirements where id = p_requirement_id;
  if v_r_ws is null then
    raise exception 'assess_version_requirement: requirement % not found', p_requirement_id using errcode='23503';
  end if;
...
comment on function public.assess_version_requirement(uuid, uuid, text, text) is
  'REQUIREMENTS 003: records or updates an assessment of a version against a requirement. Upsert per (asset_version_id, requirement_id). Applicability enforced by the frozen REQUIREMENTS 002 trigger. Requires requirement.assess.';
