-- AUTH 007: approval_rls
--
-- RLS for approval_requests, approval_request_approvers, approval_responses
-- + workflow RPCs: request_approval, respond_to_approval, cancel_approval.
--
-- All writes are RPC-only. Existing V1 triggers stay authoritative:
--   - approval_requests_target_eligibility (BEFORE INSERT) — published version check
--   - approval_responses_slot_coherence   (BEFORE INSERT) — responder identity matches slot
--   - approval_responses_no_update / no_delete           — response immutability
--
-- Canonical events (EVENT_MODEL.md §4.11):
--   approval.requested, approval.responded, approval.approved,
--   approval.rejected, approval.cancelled. (approval.expired is scheduler-driven; deferred.)
--
-- Approval does NOT change design_assets.current_version_id and does NOT
-- release anything — Approval, Current and Release remain independent.

------------------------------------------------------------------------------
-- 1. request_approval
------------------------------------------------------------------------------
-- Creates approval_requests (in_progress, MVP-simplified per STATE_MACHINES
-- §13) + full approver slot set atomically. Approver set frozen from the
-- outset. Prevents duplicate identities (structural + dual-path).
...
begin
  ...
  if v_caller is null then
    raise exception 'request_approval: authentication required' using errcode='42501';
  end if;
  if p_policy not in ('any','all') then
    raise exception 'request_approval: policy must be any|all' using errcode='22023';
  end if;
  ...
  if v_workspace_id is null then
    raise exception 'request_approval: design_asset not found' using errcode='23503';
  end if;
  if v_da_project <> p_project_id then
    raise exception 'request_approval: asset does not belong to project' using errcode='23514';
  end if;

  select design_asset_id, status, sequence into v_v_asset, v_v_status, v_v_sequence
    from public.asset_versions where id = p_version_id;
  if v_v_asset is null then
    raise exception 'request_approval: version not found' using errcode='23503';
  end if;
  if v_v_asset <> p_design_asset_id then
    raise exception 'request_approval: version does not belong to asset' using errcode='23514';
  end if;
  if v_v_status <> 'published' then
    raise exception 'request_approval: version must be published (is %)', v_v_status using errcode='23514';
  end if;
  ...
  v_approver_count :=
      coalesce(array_length(p_approver_wm_ids, 1), 0)
    + coalesce(array_length(p_approver_sh_ids, 1), 0);
  if v_approver_count < 1 then
    raise exception 'request_approval: at least one approver required' using errcode='23514';
  end if;

  foreach v_wm_id in array coalesce(p_approver_wm_ids, '{}'::uuid[]) loop
    if v_wm_id = any(v_seen_wm) then
      raise exception 'request_approval: duplicate workspace_member approver %', v_wm_id using errcode='23514';
    end if;
    v_seen_wm := v_seen_wm || v_wm_id;
    select user_id into v_probe_user
      from public.workspace_members
     where id = v_wm_id and workspace_id = v_workspace_id and status = 'active';
    if not found then
      raise exception 'request_approval: workspace_member % not active in workspace', v_wm_id using errcode='23503';
    end if;
    if v_probe_user is not null then
      if v_probe_user = any(v_seen_users) then
        raise exception 'request_approval: dual-path duplicate — user % already an approver via another path', v_probe_user using errcode='23514';
      end if;
  ...
  foreach v_sh_id in array coalesce(p_approver_sh_ids, '{}'::uuid[]) loop
    if v_sh_id = any(v_seen_sh) then
      raise exception 'request_approval: duplicate stakeholder approver %', v_sh_id using errcode='23514';
    end if;
    ...
    if not found then
      raise exception 'request_approval: stakeholder % not in workspace', v_sh_id using errcode='23503';
    end if;
    if v_probe_user is not null then
      if v_probe_user = any(v_seen_users) then
        raise exception 'request_approval: dual-path duplicate — user % already an approver via another path', v_probe_user using errcode='23514';
  ...
  insert into public.approval_requests (
    ...
    policy, status, title, description, due_at, sent_at, created_by_profile_id
  ) values (
    v_workspace_id, p_project_id, p_design_asset_id, p_version_id,
    p_policy, 'in_progress', p_title, p_description, p_due_at, now(), v_caller
  )
  returning id into v_request_id;

  foreach v_wm_id in array coalesce(p_approver_wm_ids, '{}'::uuid[]) loop
    insert into public.approval_request_approvers (
      workspace_id, approval_request_id, workspace_member_id, sort_order
    ) values (v_workspace_id, v_request_id, v_wm_id, v_sort);
    v_sort := v_sort + 1;
  end loop;
  foreach v_sh_id in array coalesce(p_approver_sh_ids, '{}'::uuid[]) loop
    insert into public.approval_request_approvers (
      workspace_id, approval_request_id, stakeholder_id, sort_order
...
------------------------------------------------------------------------------
-- 2. respond_to_approval
------------------------------------------------------------------------------
-- Caller must own the slot (resolved via wm.user_id or stakeholder.user_id).
-- Unclaimed stakeholders (user_id null) can never respond. Response is
-- immutable (existing trigger). Outcome evaluated inside the same transaction
-- under FOR UPDATE lock on the parent request.
...
begin
  ...
  if p_decision not in ('approved','rejected','changes_requested') then
    raise exception 'respond_to_approval: invalid decision %', p_decision using errcode='22023';
  end if;
  ...
  select project_id, status, policy
    into v_project_id, v_request_status, v_policy
    from public.approval_requests
   where id = v_request_id and workspace_id = v_workspace_id
   for update;
  if v_request_status <> 'in_progress' then
    raise exception 'respond_to_approval: request is % (must be in_progress)', v_request_status using errcode='23514';
  end if;
  ...
  select count(*) into v_total
    from public.approval_request_approvers where approval_request_id = v_request_id;
  select count(*),
         count(*) filter (where decision = 'approved'),
         count(*) filter (where decision in ('rejected','changes_requested'))
  ...
  if v_policy = 'any' then
    if v_approved >= 1 then
      v_outcome := 'approved';
    elsif v_responded = v_total and v_approved = 0 then
      v_outcome := 'rejected';
    end if;
  elsif v_policy = 'all' then
    if v_non_approved >= 1 then
      v_outcome := 'rejected';
...
begin
  ...
  if v_status <> 'in_progress' then
    raise exception 'cancel_approval: request is % (must be in_progress)', v_status using errcode='23514';
  end if;
  ...
  insert into public.activity_events (
    ...
    v_workspace_id, v_project_id, now(), 'approval.cancelled',
    v_caller, 'user',
...
------------------------------------------------------------------------------
-- 4. RLS policies (SELECT only; all writes RPC-only)
------------------------------------------------------------------------------
...
create policy approval_request_approvers_select on public.approval_request_approvers
  as permissive for select to authenticated
  using (
    exists (
...
create policy approval_responses_select on public.approval_responses
  as permissive for select to authenticated
  using (
    exists (
      select 1 from public.approval_requests ar
