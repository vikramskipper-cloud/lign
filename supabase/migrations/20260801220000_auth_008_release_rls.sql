-- AUTH 008: release_rls
--
-- RLS for public.releases and public.release_items + finalize_release /
-- withdraw_release RPCs.
--
-- Existing V1 triggers remain authoritative:
--   - release_items_parent_draft_mutation (BEFORE INSERT/UPDATE/DELETE on
--     release_items): mutations only while parent release is draft.
--   - releases_finalization_prerequisites_insert / _update: block a release
--     from reaching 'released' unless every included (design_asset, version)
--     has an approved approval_requests row + at least one item exists.
--
-- Added:
--   - enforce_release_status_via_rpc trigger: RLS UPDATE cannot change
--     status/released_at/withdrawn_at/withdrawn_reason unless the transaction
--     GUC lign.allow_release_status_write is set. Only the two RPCs set it.
--     Also enforces immutability of workspace_id/project_id/created_by.
--
-- Canonical events emitted:
--   - release.finalized (EVENT_MODEL.md §4.12 line 188)
--   - release.withdrawn (EVENT_MODEL.md §4.12 line 189)
-- release.created / release.item_added / release.item_removed are canonical
-- (§4.12) but not required for the finalize/withdraw workflow itself; deferred.
--
-- Independence: finalize_release does NOT touch design_assets.current_version_id,
-- does NOT publish/supersede/deprecate versions, does NOT alter approval_requests.
--
-- No structural schema change. No storage/notifications/cron/realtime/AI/nuesync.

------------------------------------------------------------------------------
-- 1. enforce_release_status_via_rpc — BEFORE UPDATE trigger
------------------------------------------------------------------------------
-- Blocks id/workspace/project/created_by mutation, and blocks any change to
-- status/released_at/withdrawn_at/withdrawn_reason unless the RPC session GUC
-- is set. finalize_release and withdraw_release set the GUC transaction-local
-- via set_config(..., true).
...
begin
  if new.id is distinct from old.id then
    raise exception 'releases.id is immutable' using errcode='23514'; end if;
  if new.workspace_id is distinct from old.workspace_id then
    raise exception 'releases.workspace_id is immutable' using errcode='23514'; end if;
  if new.project_id is distinct from old.project_id then
    raise exception 'releases.project_id is immutable' using errcode='23514'; end if;
  if new.created_by_profile_id is distinct from old.created_by_profile_id then
    raise exception 'releases.created_by_profile_id is immutable' using errcode='23514'; end if;
  ...
  if new.status is distinct from old.status and not v_permit then
    raise exception 'releases.status may only be changed via finalize_release / withdraw_release RPC (id=%)', old.id
      using errcode='42501';
  end if;
...
create trigger releases_status_via_rpc
...
begin
  ...
  select workspace_id, project_id, status, name, channel
    into v_workspace_id, v_project_id, v_status, v_name, v_channel
  ...
  -- Existing releases_finalization_prerequisites_update trigger re-verifies
  -- item count + every-item-approved on the status change below.
  ...
  insert into public.activity_events (
    ...
    v_workspace_id, v_project_id, now(), 'release.finalized',
    v_caller, 'user',
    'release', p_release_id, v_name,
    jsonb_build_object('name', v_name, 'item_count', v_item_count, 'channel', v_channel),
    '{}'::jsonb
  );
...
begin
  ...
  select workspace_id, project_id, status, name
    into v_workspace_id, v_project_id, v_status, v_name
    from public.releases
   where id = p_release_id
   for update;
  if v_workspace_id is null then
  ...
  insert into public.activity_events (
    ...
    v_workspace_id, v_project_id, now(), 'release.withdrawn',
    v_caller, 'user',
    'release', p_release_id, v_name,
...
-- Draft metadata edits (name/notes/channel/effective_at) permitted with
-- release.create. Status / released_at / withdrawn_at / withdrawn_reason /
-- workspace_id / project_id / created_by are locked by the status-gate
-- trigger (RPC-only for status transitions; immutable for identity fields).
...
------------------------------------------------------------------------------
-- 5. release_items policies (parent-draft trigger enforces timing)
------------------------------------------------------------------------------
